...
void VisualOdom::update_pose(cv::Matx44d &pose_) {

    pose_ = C_k_;

}

bool VisualOdom::initialize(){
    ...
    //** Initializing C_k_ with 0 translation and 0 rotation w.r.t. the 'some' initial co-ordinate frame
    cv::hconcat(NO_ROT_, NO_T_, C_k_);
    cv::vconcat(C_k_, TEMP_, C_k_);
   
    return true;
}

cv::Mat VisualOdom::getCameraPose(){
